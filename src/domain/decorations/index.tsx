import type { DecorationId, Localized } from '@/domain/types';

export interface DecorationProps {
  /** Primary decoration colour from the theme. */
  color: string;
  /** Unique suffix for SVG ids (gradients, masks). */
  uid: string;
}
...
const layer = { className: 'story-layer', viewBox: '0 0 1080 1920', 'aria-hidden': true } as const;
...
function Lanterns({ color, uid }: DecorationProps) {
  return (
    <svg {...layer}>
      <defs>
        <radialGradient id={`lg-${uid}`}>
          <stop offset="0" stopColor={color} stopOpacity="0.28" />
          <stop offset="1" stopColor={color} stopOpacity="0" />
        </radialGradient>
      </defs>
      <circle cx="200" cy="300" r="230" fill={`url(#lg-${uid})`} />
      <circle cx="540" cy="200" r="200" fill={`url(#lg-${uid})`} />
      <circle cx="880" cy="360" r="230" fill={`url(#lg-${uid})`} />
...
function Mosque({ color, uid }: DecorationProps) {
  ...
  return (
    <svg {...layer}>
      ...
      <rect x="0" y="1450" width="1080" height="470" fill={`url(#${gid})`} />
...
function Kaaba({ color, uid }: DecorationProps) {
  ...
  return (
    <svg {...layer}>
      ...
      <ellipse cx="540" cy="1780" rx="420" ry="200" fill={`url(#${gid})`} />
    ...
    <svg {...layer}>
      ...
      {bulbs.map((x, i) => (
        <g key={x}>
          ...
          <circle cx={x} cy={y(x) + 60} r="60" fill={`url(#${gid})`} />
          {i % 2 === 0 ? (
...
function Sunrise({ color, uid }: DecorationProps) {
  ...
  return (
    <svg {...layer}>
      ...
      <circle cx="540" cy="360" r="420" fill={`url(#${gid})`} />
      <g stroke={color} strokeWidth="3" strokeLinecap="round" opacity="0.45">
        {rays.map((a) => {
          ...
          return (
            <line
              key={a}
              x1={540 + r1 * Math.cos(rad)}
              y1={360 + r1 * Math.sin(rad)}
              x2={540 + r2 * Math.cos(rad)}
              y2={360 + r2 * Math.sin(rad)}
            />
          );
        })}
      </g>
...
/**
 * Decoration registry. To add one, write a component that draws in the
 * 1080x1920 coordinate space and register it here plus in `DecorationId`.
 */
