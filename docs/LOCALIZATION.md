...
### Story card text — 43 done, 262 to source

**Status.** `Post.translations` carries a rendering per locale, `postTranslation()` resolves it, and
the 44 `kind: 'greeting'` cards are written in all five new languages. Arabic deliberately has no
entry anywhere: the card already carries the Arabic, so the line beneath it stays English rather
than repeating the text.

| Kind     | Cards | Rendered | Why not                                  |
| -------- | ----- | -------- | ---------------------------------------- |
| greeting | 44    | 43       | authored well-wishes — ours to translate |
| quran    | 129   | 0        | scripture; needs a licensed edition      |
| hadith   | 65    | 0        | transmitted; needs a licensed edition    |
| dua      | 40    | 0        | transmitted                              |
| dhikr    | 27    | 0        | transmitted                              |

A test in `content.test.ts` enforces the line: only greetings may carry `translations`. Adding a
hand-written Indonesian rendering of a Quran verse now fails the build, on purpose.

**How the remaining 262 get done.** Not by translating them here — by mapping each card to an
edition that already exists:

- **Quran, 129 cards.** They are generated by `scripts/gen-quran-posts.mjs` from verse references,
  so the generator gains a per-locale translation source and fills them. The route is the one in
  section 3: QuranEnc editions, permitted with attribution, covering all five languages.
- **Hadith, 65 cards.** The `fawazahmed0` dataset already in use has French, Indonesian and Urdu
  editions; Malay and Thai come from HadeethEnc. Match on collection and number.
- **Duaa and dhikr, 67 cards.** Mostly transmitted text; same sources, matched by hand where no
  reference exists.

Each needs its attribution recorded next to the text, as section 3 requires.

### The original note on why this cannot be written by hand

Each post carries a `translation` field, currently English. 305 × 4 = 1,220 strings — but they are
Quran verses, hadith, and duaa. **These cannot be authored or machine-translated.** A rendering of a
verse into Indonesian has to come from an established, attributable translation, or it is a new
interpretation of scripture published under the app's name. Take them from the same source as the
reader (section 3) so a verse reads identically in the card and in the reader.
