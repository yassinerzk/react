  ...
  return (
    <div
      ...
      style={{
        width: STORY_WIDTH,
        height: STORY_HEIGHT,
        background: gradientToCss(theme.gradient),
        color: palette.text,
      }}
    >
      {background ? (
        <>
          <img
            className="story-layer story-photo"
            src={backgroundSrc(background.id)}
            alt=""
            decoding="async"
          />
          <div className="story-layer" style={{ background: gradientToCss(SCRIMS[background.scrim]) }} />
        </>
      ) : (
